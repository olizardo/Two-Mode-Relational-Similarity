...
As Figure~\ref{fig:tmrs-events} shows, the two-mode relational similarity approach yields results that are somewhat different from those obtained using the generalized relational similarities, but that are in some ways closer to the target clusters from Doreian et al.'s blockmodel solution \citeyearpar{doreian2004}. As with the Kovacs approach, events 1-6 and events 10-14 end up closely spaced and assigned to their own cluster. Events 7 and 8 fall into their own cluster as would be expected given the Doreian et al. partition. Event 9, however, is grouped with the event 10-14 cluster. In all, the two-mode relational similarity approach seems more effective at avoiding singleton clusters than the generalized similarity measure. 
  ...
  \centering
     \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=1.0\textwidth]{Plots/dend-actors-grs.png}
        \caption{}
        \label{fig:dend-actors-grs}
    \end{subfigure} 
     \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=1.0\textwidth]{Plots/dend-actors-tmrs.png}
        \caption{}
        \label{fig:dend-actors-tmrs}
    \end{subfigure} 
     \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=1.0\textwidth]{Plots/dend-events-grs.png}
        \caption{}
        \label{fig:dend-events-grs}
    \end{subfigure} 
     \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=1.0\textwidth]{Plots/dend-events-tmrs.png}
        \caption{}
        \label{fig:dend-events-tmrs}
    \end{subfigure} 
    \caption{Actor and event dendrograms obtained from a hierarchical clustering partition based on the complete linkage merge criterion. Actors and events are colored based on cluster membership from a three-cluster cut.}
    \label{fig:dends}
 \end{figure}


\subsection{Correspondence between similarities}
Figures \ref{fig:actors} and \ref{fig:events} suggest that the solutions returned by Kovacs's generalized relational similarities and the two-mode relational similarities are themselves similar. To get a more precise estimate of exactly \textit{how} similar, we can compare the dendrograms produced by a (complete linkage) hierarchical clustering of both actors and events derived from each of the distances. These are shown in Figure~\ref{fig:dends}: Figures~\ref{fig:dend-actors-grs} and~\ref{fig:dend-actors-tmrs} show the actor dendrograms, and Figures~\ref{fig:dend-events-grs} and~\ref{fig:dend-events-tmrs} show the corresponding event dendrograms. 

A simple measure from biostatistics, the cophenetic correlation distance ($c$), can be used to quantify how similar each pair of dendrograms is \citep{sokal1962comparison}.\footnote{The cophenetic correlation is the Pearson correlation between the cophenetic distance matrices implied by a hierarchical clustering partition (or in , where the cophenetic distance between objects is defined as the number of branches that have to be upwardly traversed before the two objects are merged into the same branch.} For the actor dendrograms $c = 0.98$, and for the event dendrograms $c = 0.99$, indicating that the two distance measures return practically indistinguishable object partitions for both actors and events. 
